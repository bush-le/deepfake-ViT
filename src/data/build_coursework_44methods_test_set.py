"""
Script to build the comprehensive scaled-up test suite strictly covering ALL 44 CourseWork Methods from Quang Manh.
Zero leakage verified against train_v5_weakfix_v3.csv.
"""
...
OUT_DIR = Path('/workspace/data/zero_leakage_benchmark_fixed')
...
# 2. Collect candidates for ALL Coursework Methods
candidate_files = []

# A. From test_data_v3 (38 clean methods + starganv2 + CollabDiff + real)
p_test_v3 = Path('/workspace/data/test_data_v3')
for d in sorted(p_test_v3.iterdir()):
    if d.is_dir():
        m_name = d.name
        label = 0 if m_name == 'real' else 1
        imgs = list(d.glob('**/*.png')) + list(d.glob('**/*.jpg'))
        for p in imgs:
            candidate_files.append({'path': str(p), 'method': m_name, 'label': label, 'source': 'test_data_v3'})

# B. From df-40-test-full for starganv2 & CollabDiff clean holdouts
for m_name in ['starganv2', 'CollabDiff']:
    m_dir = Path(f'/workspace/data/df-40-test-full/{m_name}/fake')
    if m_dir.exists():
        imgs = list(m_dir.glob('*.png')) + list(m_dir.glob('*.jpg'))
        for p in imgs:
            candidate_files.append({'path': str(p), 'method': m_name, 'label': 1, 'source': 'df-40-test-full'})

# C. From deep-fake-face-swap holdout
dffs_dir = Path('/workspace/data/deep-fake-face-swap')
if dffs_dir.exists():
    imgs = list(dffs_dir.glob('**/*.jpg')) + list(dffs_dir.glob('**/*.png'))
    for p in imgs[:3000]:
        if str(p) not in train_paths:
            candidate_files.append({'path': str(p), 'method': 'deepfake_faceswap', 'label': 1, 'source': 'deep-fake-face-swap'})

# D. From kaggle for sfhq_studio & stable_diffusion
sfhq_dir = Path('/workspace/data/kaggle/philosopher0808/real-vs-ai-generated-faces-dataset/versions/1/data_source/data_source/sfhq/studio')
if sfhq_dir.exists():
    imgs = list(sfhq_dir.glob('*.jpg'))
    for p in imgs[:3000]:
        if str(p) not in train_paths:
            candidate_files.append({'path': str(p), 'method': 'sfhq_studio', 'label': 1, 'source': 'kaggle_sfhq'})

sd_dir = Path('/workspace/data/kaggle/philosopher0808/real-vs-ai-generated-faces-dataset/versions/1/data_source/data_source/stable_diffusion')
if sd_dir.exists():
    imgs = list(sd_dir.glob('*.jpg'))
    for p in imgs[:3000]:
        if str(p) not in train_paths:
            candidate_files.append({'path': str(p), 'method': 'stable_diffusion', 'label': 1, 'source': 'kaggle_sd'})

# E. Real candidates from FFHQ & FaceForensics++ holdouts
ffhq_dir = Path('/workspace/data/kaggle/philosopher0808/real-vs-ai-generated-faces-dataset/versions/1/data_source/data_source/ffhq')
if ffhq_dir.exists():
    imgs = list(ffhq_dir.glob('*.jpg'))
    for p in imgs[:30000]:
        if str(p) not in train_paths:
            candidate_files.append({'path': str(p), 'method': 'real', 'label': 0, 'source': 'ffhq_real'})

ff_dir = Path('/workspace/data/FaceForensics++/original_sequences/youtube/c23/frames')
if ff_dir.exists():
    imgs = list(ff_dir.glob('*/*.png'))
    for p in imgs[:10000]:
        if str(p) not in train_paths:
            candidate_files.append({'path': str(p), 'method': 'real', 'label': 0, 'source': 'ff++_real'})

print(f'Total candidate frames collected: {len(candidate_files):,}')

# 3. MD5 Hashing & 3-Tier Leak Filter
...
print(f'\nTotal Clean Fake Frames across all Coursework Methods: {len(df_fake_clean):,}')
...
# Save CSVs
bal_csv_path = OUT_DIR / 'test_coursework_44methods_balanced_zero_leakage.csv'
full_csv_path = OUT_DIR / 'test_coursework_44methods_full_zero_leakage.csv'
