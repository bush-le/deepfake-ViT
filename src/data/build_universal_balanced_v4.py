...
for base_dir in ["/workspace/data/test_data_v3", "/workspace/data/df-40-test-full"]:
...
# 1.3 Celeb-DF Real
for f in sorted(glob.glob("/workspace/hoangtuan/deepfake-ViT/data/processed/celeb_df_extracted/*.png")):
...
for f in sorted(glob.glob("/workspace/data/FaceForensics++/original_sequences/youtube/c23/frames/*/*.png")):
    if os.path.exists(f) and os.path.getsize(f) > 0 and os.access(f, os.R_OK):
