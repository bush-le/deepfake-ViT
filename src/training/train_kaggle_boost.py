"""
================================================================================
Train & Evaluation Pipeline: Kaggle MidJourney Boost (12k) on DINOv3 ViT
...
def train_epoch(model, loader, criterion, optimizer, scaler, device, ema=None):
    model.train()
    total_loss = 0.0
    correct = 0
    total = 0
...
def run_training(args):
    print("=" * 80)
    print("🚀 TRAINING DINOv3 ViT: KAGGLE MIDJOURNEY ENHANCEMENT PIPELINE")
    print("=" * 80)
    ...
    run_name = f"{ts}_kaggle_midjourney_boost"
    ...
    global_ckpt_dir = PROJECT_ROOT / "experiments" / "checkpoints" / "kaggle_midjourney_boost"
    ...
    print(f"📊 Loaded Boost Dataset: {len(df_all):,} samples ({args.train_csv})")
    
    # Train / Val Split (90% Train, 10% Val)
    df_all = df_all.sample(frac=1.0, random_state=args.seed).reset_index(drop=True)
    val_size = int(len(df_all) * 0.10)
    df_val = df_all.iloc[:val_size]
    df_train = df_all.iloc[val_size:]

    print(f"   • Train Split : {len(df_train):,} samples")
    print(f"   • Val Split   : {len(df_val):,} samples")

    train_ds = DeepfakeCSVDataset(df_train, transform=get_train_transforms(args.img_size))
    val_ds = DeepfakeCSVDataset(df_val, transform=get_eval_transforms(args.img_size))
    ...
    # 3. Model Build & Checkpoint Initialization
    weights_candidates = [
        PROJECT_ROOT / "models" / "dinov3_small" / "model.safetensors",
        PROJECT_ROOT / "experiments" / "checkpoints" / "weights" / "model.safetensors",
    ...
    for epoch in range(1, args.epochs + 1):
        t0 = time.time()
        tr_loss, tr_acc = train_epoch(model, train_loader, criterion, optimizer, scaler, device, ema)
        ...
        torch.save(last_ckpt_payload, ckpt_dir / "kaggle_midjourney_boost_last.pt")

        # Save Best Checkpoint
        if val_res["auc"] > best_val_auc:
            best_val_auc = val_res["auc"]
            best_epoch = epoch
            torch.save(last_ckpt_payload, ckpt_dir / "kaggle_midjourney_boost_best.pt")
    ...
    print("\n" + "=" * 80)
    print("🧪 EVALUATING ON FIXED ZERO-LEAKAGE TEST SUITE (2,354 SAMPLES)")
    print("=" * 80)
    df_test = pd.read_csv(args.test_csv)
    test_ds = DeepfakeCSVDataset(df_test, transform=get_eval_transforms(args.img_size))
    ...
    df_method_perf.to_csv(results_dir / "kaggle_boost_per_method_accuracy.csv", index=False)
    print(f"\n💾 Saved full report to: {report_path}")
    print(f"💾 Saved per-method metrics to: {results_dir / 'kaggle_boost_per_method_accuracy.csv'}")

    return final_report


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train DINOv3 ViT on Kaggle Boost Dataset.")
    parser.add_argument("--train-csv", type=str, default=str(DATA_ROOT / "zero_leakage_benchmark_fixed/train_kaggle_midjourney_boost_12k.csv"))
    parser.add_argument("--test-csv", type=str, default=str(DATA_ROOT / "zero_leakage_benchmark_fixed/test_balanced_fixed_zero_leakage.csv"))
