...
def get_group_key(path, file_to_hash):
...
def sample_balanced_fakes(pool_dict, target_total, active_fake_methods):
    sampled = []
...
def sample_balanced_reals(pool_dict, target_total):
    ...
    if not sources or target_total <= 0:
        return []
...
def main():
    ...
    if not candidate_files:
        print("⚠️ No candidate images found on disk. Exiting.")
        return
    ...
    for f in candidate_files:
        ...
        g_key = get_group_key(f, file_to_hash)
        s = assign_group_split(g_key)

        m_name, is_real = file_to_info.get(f, ("unknown", False))
        if is_real:
            if s == "train":
                train_reals[m_name].append(f)
            elif s == "val":
                val_reals[m_name].append(f)
            else:
                test_reals[m_name].append(f)
        else:
            if s == "train":
                train_fakes[m_name].append(f)
            elif s == "val":
                val_fakes[m_name].append(f)
            else:
                test_fakes[m_name].append(f)
    ...
    train_fake_samples = sample_balanced_fakes(train_fakes, TRAIN_TOTAL_FAKE, active_fake_methods)
    val_fake_samples = sample_balanced_fakes(val_fakes, VAL_TOTAL_FAKE, active_fake_methods)
    test_fake_samples = sample_balanced_fakes(test_fakes, TEST_TOTAL_FAKE, active_fake_methods)
