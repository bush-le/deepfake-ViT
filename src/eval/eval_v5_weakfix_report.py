"""
Regenerate v5_weakfix report files from the saved checkpoint without retraining.
Loads experiments/checkpoints/exp05_v5_weakfix/best_model.pt, evaluates on the
zero-leakage test set, writes report JSON + per-method + vs-baseline CSVs.
"""
import os, sys, json
import pandas as pd
import torch
...
DATA_ROOT = Path(os.getenv("DF40_ROOT", ROOT / "data"))
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from src.training.finetune_v5_weakfix import (DeepfakeDataset, get_eval_transforms, evaluate)
from torch.utils.data import DataLoader
from src.models.dinov3_vit import build_dinov3_classifier

CKPT = ROOT / "experiments/checkpoints/exp05_v5_weakfix/best_model.pt"
TEST_CSV = DATA_ROOT / "zero_leakage_benchmark_fixed/test_balanced_fixed_zero_leakage.csv"
...
def main():
    ...
    (ROOT / "experiments/results/v5_weakfix_training_report.json").write_text(
        json.dumps(report, indent=2))
    df_method.to_csv(ROOT / "experiments/results/v5_weakfix_per_method_accuracy.csv", index=False)
    merged.to_csv(ROOT / "experiments/results/v5_weakfix_vs_baseline.csv", index=False)
    print("Saved report JSON + per-method CSV + vs-baseline CSV")
